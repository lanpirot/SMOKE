...
function csvData = runLoop(models, csvData, csvFile, TMP_MODEL_SAVE_PATH, args)
    ...
    for m = 5353:10000
        ...
        try
            ...
            blocks = find_system(s, 'LookUnderMasks', 'all', 'FollowLinks', 'on', 'MatchFilter', @Simulink.match.allVariants, 'IncludeCommented', 'on');
...
function args = get_args()
    ...
    args = {...
        ...
        'sffunctions',            1, ...
        'sflabels',               1, ...
        'removemodelreferences',  0, ... %changes the count of model elements, even though, technically the model structure is not changed
        'recursemodels',          1, ...
        'customdatatypes',        1, ...
        'completeModel',          1};
    %OPTION 2
